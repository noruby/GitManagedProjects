...
#Energy_curvature_valence_vector <-  grad_k_midpoint(grad_k_midpoint(energy_valence_vector0))
#effective_mass_valence <- Plank_constant_bar^2/Energy_curvature_valence_vector[round(div_num_ka/2)]
#Energy_curvature_conduction_vector <- grad_k_midpoint(grad_k_midpoint(energy_conduction_vector0))
...
krange <- c(-pi, pi)
eVrange <- c(-2,6)
plot(ka_vector0, energy_valence_vector0/(1.60218*10^-19), xlim=krange, ylim=eVrange, xlab="ka (rad)", ylab= "energy (eV)", col="red", type="l")  
par(new=T)
plot(ka_vector0, energy_conduction_vector0/(1.60218*10^-19), xlim=krange, ylim=eVrange, xlab="", ylab="", col="blue", type="l")  
par(new=T)
plot(ka_vector0, energy_difference_vector0/(1.60218*10^-19), xlim=krange, ylim=eVrange, xlab="", ylab="", col="green", type="l")  
dev.off() 
}
...
phase_valence_vector <- phase_valence(ka_vector0)
phase_conduction_vector <- phase_conduction(ka_vector0)
...
plot(ka_vector0, Re(d_vv) , xlim=krange, xlab="ka (rad)", ylab= "dipole_transition", ylim=drange, col="red", type="l")  
par(new=T)
plot(ka_vector0,Re(d_cc), xlim=krange, xlab="", ylab="", ylim=drange, col="blue", type="l")
par(new=T)
plot(ka_vector0,Re(d_vc), xlim=krange, xlab="", ylab="", ylim=drange, col="green", type="l")
par(new=T)
plot(ka_vector0,Im(d_vc), xlim=krange, xlab="", ylab="", ylim=drange, col="green", type="l", lty="dashed")
dev.off()   
}
...
diff_micro_polarization_length <- function(E, mpl, nvl, ncl){
	dmpl <- (1i* energy_difference_vector0 *mpl - elementary_charge* E*( grad_k_midpoint(mpl) + 1i*(d_cc-d_vv)*mpl -1i*d_vc*(ncl- nvl) ) ) * delta_t/ Plank_constant_bar
...
for(i in 1:(div_num_ka)){
	Current_valence_length <-  Current_valence_length - elementary_charge/Plank_constant_bar* grad_energy_valence_vector0[i] * nvl_kt[i,] *delta_k
	Current_conduction_length <- Current_conduction_length - elementary_charge/Plank_constant_bar* grad_energy_conduction_vector0[i] * ncl_kt[i,] *delta_k 
	Polarization_current_length <- Polarization_current_length + elementary_charge/Plank_constant_bar* energy_difference_vector0[i]* 2* Im(mpl_kt[i,] * Conj(d_vc[i])) *delta_k
...
gauge<-"velocity"
if(gauge=="velocity"){
# ed_t: energy_difference_vector_t
# ged_t: grad_energy_difference_vector_t
diff_number_valence_velocity <- function(A, mpv, ed_t){
	dnvv <- 2*elementary_charge/ Plank_constant_bar^2 * A *ed_t* Re(mpv*Conj(d_vc)) * delta_t 
	#dnvv <- dnvv - (nvv-1) /relaxation_constant_velocity *delta_t
	return (dnvv)
}
diff_number_conduction_velocity <- function(A, mpv, ed_t){
	dncv <- -2*elementary_charge/Plank_constant_bar^2 * A *ed_t* Re(mpv*Conj(d_vc)) * delta_t
	#dncv <- dncv - ncv /relaxation_constant_velocity *delta_t
	return (dncv)
}
diff_micro_polarization_velocity <- function(A, mpv, nvv, ncv, ed_t, ged_t){
	dmpv <- ( 1i *ed_t *mpv /Plank_constant_bar
	-1i* elementary_charge* A /Plank_constant_bar^2 * ged_t*mpv  
	+ elementary_charge* A /Plank_constant_bar^2 * ed_t* d_vc * (ncv-nvv) ) * delta_t
...
ka_vector_t <- rep(0, div_num_ka)
ev_t <- rep(0, div_num_ka)
ec_t <- rep(0, div_num_ka)
ed_t <- rep(0, div_num_ka)
gev_t <- rep(0, div_num_ka)
gec_t <- rep(0, div_num_ka)
ged_t <- rep(0, div_num_ka)
...
for( j in 1:(div_num_t) ){
	...
	ka_vector_t <- ka_vector0 + elementary_charge* A/ Plank_constant_bar*lattice_constant_a_of_GaSe
	ka_vector_t <- ka_vector_t %% (2*pi)
	ev_t <- energy_valence(ka_vector_t)
	ec_t <- energy_conduction(ka_vector_t)
	ed_t <- ec_t - ev_t
	gev_t <- grad_k_midpoint(ev_t)
	gec_t <- grad_k_midpoint(ec_t)
	ged_t <- gec_t - gev_t
	
	dnvv_0 <- diff_number_valence_velocity(A, mpv, ed_t)
	dncv_0 <- diff_number_conduction_velocity(A, mpv, ed_t)
	dmpv_0 <- diff_micro_polarization_velocity(A, mpv, nvv, ncv, ed_t, ged_t)

	dnvv_1 <- diff_number_valence_velocity(A_0, mpv+dmpv_0/2, ed_t)
	dncv_1 <- diff_number_conduction_velocity(A_0, mpv+dmpv_0/2, ed_t)
	dmpv_1 <- diff_micro_polarization_velocity(A_0, mpv+dmpv_0/2, nvv+dnvv_0/2, ncv+dncv_0/2, ed_t, ged_t)
	
	dnvv_2 <- diff_number_valence_velocity(A_1, mpv+dmpv_1/2, ed_t)
	dncv_2 <- diff_number_conduction_velocity(A_1, mpv+dmpv_1/2, ed_t)
	dmpv_2 <- diff_micro_polarization_velocity(A_1, mpv+dmpv_1/2, nvv+dnvv_1/2, ncv+dncv_1/2, ed_t, ged_t)

	dnvv_3 <- diff_number_valence_velocity(A_2, mpv+dmpv_2, ed_t)
	dncv_3 <- diff_number_conduction_velocity(A_2, mpv+dmpv_2, ed_t)
	dmpv_3 <- diff_micro_polarization_velocity(A_2, mpv+dmpv_2, nvv+dnvv_2, ncv+dncv_2, ed_t, ged_t)
	...
	for(i in 1:(div_num_ka)){
  	 Current_valence_velocity[j+1] <-  Current_valence_velocity[j+1] - elementary_charge/Plank_constant_bar* gev_t[i] * nvv_kt[i,j+1] *delta_k
    Current_conduction_velocity[j+1] <- Current_conduction_velocity[j+1] - elementary_charge/Plank_constant_bar* gec_t[i] * ncv_kt[i,j+1] *delta_k 
	  Polarization_current_velocity[j+1] <- Polarization_current_velocity[j+1] + elementary_charge/Plank_constant_bar* ed_t[i]*2* Im(mpv_kt[i,j+1] * Conj(d_vc[i])) *delta_k
  	  Energy_free_velocity[j+1] <- Energy_free_velocity[j+1] +(ev_t[i] * nvl_kt[i,j+1] + ec_t[i] * ncl_kt[i,j+1] ) *delta_k
	  Energy_interaction_velocity[j+1] <- Energy_interaction_velocity[j+1] -elementary_charge* A_vector/Plank_constant_bar*(nvv_kt[i,j+1]*gev_t[i] + ncv_kt[i,j+1]*gec_t[i] - ed_t[i] *2*Im(Conj(d_vc[i])*mpv_kt[i,j+1])) *delta_k
  }
  print(j/div_num_t)
}
...
Erange <- c( -max(E_vector), max(E_vector) ) 
trange <- c(-range_t/2, range_t/2)*10^15
png("./debug_ElectricF_comparison.png", width = 700, height = 1000)  
split.screen(c(2,1))
screen(1);
plot(t_vector*10^15, Re(E_vector), xlab="", ylab= "Electric field (arb. unit)", xlim=trange, axes=FALSE, yaxt="n",col="black", type="l")  
par(new=T)
plot(t_vector*10^15, Re(E_HHG_length), xlab="t (fs)", ylab="", xlim=trange,  ylim=E_HHGrange, col="red", type="l", lwd=3)  
par(new=T)
plot(t_vector*10^15, Re(Current_length), xlab="", ylab="", xlim=trange, ylim=E_HHGrange, axes=FALSE, yaxt="n", col="blue", type="l")  
par(new=T)
plot(t_vector*10^15, Re(Polarization_current_length), xlab="", ylab= "", xlim=trange, ylim=E_HHGrange, axes=FALSE, yaxt="n", col="green", type="l")  
...
plot(t_vector*10^15, Re(E_vector), xlab="", ylab= "Electric field (arb. unit)", xlim=trange, axes=FALSE, yaxt="n",col="black", type="l")  
par(new=T)
plot(t_vector*10^15, Re(E_HHG_velocity), xlab="t (fs)", ylab="", xlim=trange,  ylim=E_HHGrange, col="red", type="l", lwd=3)  
par(new=T)
plot(t_vector*10^15, Re(Current_velocity), xlab="", ylab="", xlim=trange, ylim=E_HHGrange, axes=FALSE, yaxt="n", col="blue", type="l")
par(new=T)
plot(t_vector*10^15, Re(Polarization_current_velocity), xlab="", ylab= "", xlim=trange, ylim=E_HHGrange, axes=FALSE, yaxt="n", col="green", type="l")  
...
frange <- c(0, 10)
png("./debug_spectrum_comparison.png", width = 700, height = 1000) 
split.screen(c(1,2))
screen(1);
#plot(frequency/carrier_frequency, abs(E_spectrum)^2, xlim=frange, log="y", xlab="f/carrier frequency", ylab="power (arb. unit)", axes=FALSE, yaxt="n", col="black", type="l", lwd = 1)  
#par(new=T)
plot(frequency/carrier_frequency, abs(E_HHG_length_spectrum)^2, xlim=frange, ylim=E_HHGrange, log="y", xlab="f/carrier frequency", ylab="power (arb. unit)", col="red", type="l", lwd=3)  
par(new=T)
...
par(new=T)
plot(frequency/carrier_frequency, abs(Polarization_current_length_spectrum)^2, xlim=frange, ylim=E_HHGrange, log="y", axes=FALSE, yaxt="n",xlab="", ylab= "", col="green", type="l")  
abline(v=0:15, col='black', lty="dotted")
screen(2);
plot(frequency/carrier_frequency, abs(E_HHG_velocity_spectrum)^2, xlim=frange, ylim=E_HHGrange, log="y", xlab="f/carrier frequency", ylab="power (arb. unit)", col="red", type="l", lwd=3)  
par(new=T)
plot(frequency/carrier_frequency, abs(Current_velocity_spectrum)^2, xlim=frange, ylim=E_HHGrange, log="y", axes=FALSE, yaxt="n",xlab="", ylab= "", col="blue", type="l")  
par(new=T)
plot(frequency/carrier_frequency, abs(Polarization_current_velocity_spectrum)^2, xlim=frange, ylim=E_HHGrange, log="y", axes=FALSE, yaxt="n",xlab="", ylab= "", col="green", type="l")  
...
if(0){
print("Exporting debug_np_comparison.gif ...")
numt <- round( 5*10^-15 /delta_t )
nplim=c(-1.2, 1.2)
library(animation)
split.screen(c(2,2))
saveGIF({
  ...
  for (i in seq(1,(div_num_t+1), by=numt)) {
    ...
    screen(1); plot(ka_vector0, nvl_kt[,i], xlim=c(-pi,pi), ylim=nplim, xlab="BZ", ylab= "numbers", main=.main,col="green", type="l")
    screen(2); plot(ka_vector0, Re(mpl_kt[,i]), xlim=c(-pi,pi), ylim=nplim, xlab="BZ", ylab= "polarization", main=.main, col="blue", type="l")  
    screen(3); plot(ka_vector0, nvv_kt[,i], xlim=c(-pi,pi), ylim=nplim, xlab="BZ", ylab= "number", main=.main,col="green", type="l")
    screen(4); plot(ka_vector0, Re(mpv_kt[,i]), xlim=c(-pi,pi), ylim=nplim, xlab="BZ", ylab= "polarization", main=.main, col="blue", type="l")
    }
}, movie.name="debug_np_comparison.gif", ani.width = 800, ani.height = 800, interval=0.2)
}
...
saveGIF({
  ...
  for (i in seq(1,(div_num_t+1), by=numt)) {
    .main = paste(t_vector[i]*10^15,"(fs)", seq="")
    plot(ka_vector0, nvl_kt[,i], xlim=c(-pi,pi), ylim=nplim, xlab="BZ", ylab= "numbers", main=.main,col="green", type="l")
    par(new=T)
    plot(ka_vector0, ncl_kt[,i], xlim=c(-pi,pi), ylim=nplim, xlab="", ylab= "", main=.main, col="blue", type="l")
    par(new=T)    
    plot(ka_vector0, Re(mpl_kt[,i]), xlim=c(-pi,pi), ylim=nplim, xlab="BZ", ylab= "polarization", main=.main,col="red" , type="l")
  }
...
saveGIF({
  ...
  for (i in seq(1,(div_num_t+1), by=numt)) {
    .main = paste(t_vector[i]*10^15,"(fs)", seq="")
    plot(ka_vector0, nvv_kt[,i], xlim=c(-pi,pi), ylim=nplim, xlab="BZ", ylab= "number/polarization", main=.main,col="green", type="l")
    par(new=T)
    plot(ka_vector0, ncv_kt[,i], xlim=c(-pi,pi), ylim=nplim, xlab="", ylab= "", main=.main, col="blue", type="l")
    par(new=T)    
    plot(ka_vector0, Re(mpv_kt[,i]), xlim=c(-pi,pi), ylim=nplim, xlab="BZ", ylab= "", main=.main,col="red" , type="l")
  }
...
gauge <- "length" 
basis <- "not Wannier"
if(gauge=="length" && basis=="Wannier"){
Blavais_lattice <- rep(-lattice_constant_a_of_GaSe*div_num_ka/2, by=lattice_constant_a_of_GaSe, length=div_num_ka)
rho_WL <- array(0, dim=c(2,2, div_num_ka, div_num_ka)) 
FourrierT_BtoW <- function(o){
	
} 
Wenergy <- array(0, dim=c(2, div_num_ka)) 
Wd <- array(0, dim=c(2, div_num_ka, div_num_ka) 
Wenergy[1,] <- FourrierT_BtoW(energy_valence)
Wenergy[2,] <- FourrierT_BtoW(energy_conduction)
Wd[1,1,] <- FourrierT_BtoW(d_vv)
Wd[2,2,] <- FourrierT_BtoW(d_cc)
Wd[1,2,] <- FourrierT_BtoW(d_vc)
Wd[2,1,] <- FourrierT_BtoW(Conj(d_vv))

diff_density_operator_WL <- function(E, rho_WL){
	d_rho_WL <- array(0, dim=c(2,2, div_num_ka, (div_num_t+1))) 
	for(i in 1:2){
	for(j in 1:2){
	for(k in 1:div_num_ka){
	for(l in 1:div_num_ka){
	  for(m in 1:div_num_ka){
	  	  for(n in 1:2){
	       d_rho_WL[i,j,k,l] <- d_rho_WL[i,j,k,l] - elementary_charge * E* ( Wd[i,n,m-k]*rho_WL[n,j,m,l] - rho_WL[i,n,k,m] *Wd[n,j,l-m] )
	     }
	     d_rho_WL[i,j,k,l] <- d_rho_WL[i,j,k,l]  + Wenergy[i,m-k] * rho_WL[i,j,m,l] - rho_WL[i,j,k,m] Wenergy[i,l-m]
	  }
	  d_rho_WL[i,j,k,l] <- d_rho_WL[i,j,k,l] - elementary_charge * E* (Blavais_lattice[k] -Blavais_lattice[l]) *rho_WL[i,j,k,l] 
	}
	}
	}
	}
		  return (d_rho_WL/ 1i/ Planck_constant_bar)
}

d_rho_WL0 <- array(0, dim=c(2,2, div_num_ka, div_num_ka)) 
d_rho_WL1 <- array(0, dim=c(2,2, div_num_ka, div_num_ka))  
d_rho_WL2 <- array(0, dim=c(2,2, div_num_ka, div_num_ka)) 
d_rho_WL3 <- array(0, dim=c(2,2, div_num_ka, div_num_ka)) 
#solving Lioville equation with the length gauge and bloch-state representation (SBE) by the 4th Runge-Kutta method
print("starting calculation with length gauge")
for( j in 1:(div_num_t) ){
	E <- E_vector2[2*j-1]
   ...
   d_rho_WL0 <- diff_density_operator_WL(E, rho_WL)
   d_rho_WL1 <- diff_density_operator_WL(E, rho_WL+d_rho_WL0/2)
   d_rho_WL2 <- diff_density_operator_WL(E, rho_WL+d_rho_WL1/2)
   d_rho_WL3 <- diff_density_operator_WL(E, rho_WL+d_rho_WL2)   

   rho_WL <- rho_WL + (d_rho_WL0+2*d_rho_WL1+2*d_rho_WL2+d_rho_WL3)/6
...
saveGIF({
  ...
  for (i in seq(1,(div_num_t+1), by=numt)) {
    .main = paste(t_vector[i]*10^15,"(fs)", seq="")
    ka_vector0 <- ka_to_ka(ka_vector0, A_vector[i])
    plot(ka_vector0, energy_valence(ka_vector0)/(1.60218*10^-19), xlim=c(-pi,pi), ylim=erange, xlab="BZ", ylab= "energy (eV)", main=.main, col="green", type="l")
    par(new=T)
    plot(ka_vector0, energy_conduction(ka_vector0)/(1.60218*10^-19), xlim=c(-pi,pi), ylim=erange, xlab="", ylab= "", main=.main, col="blue", type="l")  
    par(new=T)
    plot(ka_vector0, micro_current_valence(ka_vector0), xlim=c(-pi,pi), ylim=jrange, xlab="BZ", ylab= "energy (eV)", main=.main, col="green", type="p")
    par(new=T)
    plot(ka_vector0, micro_current_conduction(ka_vector0), xlim=c(-pi,pi), ylim=jrange, xlab="", ylab= "", main=.main, col="blue", type="p")  
    }
