...
#modified in 11-12 Sep. 2017
options(scipen=0)

#Physical constants 
Plank_constant <- 1.054571800 * 10^-34 #m^2 kg / s; (h bar)
...
#E_vector2[201:(2*div_num_t+1)] <-  rep(E_amplitude_max, 2*div_num_t-749)
...
#band structure of GaSe (5 band model assuming the Tight binding model (pseudo-2D))
band_number <- 5
Energies_gamma_point <- c(3.5, 2.11, 0, -1.27, -1.59) # eV 
band_widths <- c(0.5, 1.5, 1, 1, 1)  # eV 
flipping_numbers <- c(-1, 1, -1, -1, -1)

band_structure <- array(0, dim=c(band_number, band_number, div_num_ka))
for(i in 1:band_number){
band_structure[i,i,] <- band_widths[i] *flipping_numbers[i] * (1 - cos(ka_vector) )/2 +  Energies_gamma_point[i] #eV
}

for(i in 1:band_number){
for(j in 1:band_number){
	if(i!=j){
		band_structure[i,j,]  <-  band_structure[i,i,] - band_structure[j,j,]
	}
}
}
...
erange=c(-3,5)
for(i in 1:band_number){
	plot(ka_vector, band_structure[i,i,], xlab="ka (rad)", ylab="energy (eV)", ylim=erange,col="black", type="l") 
	if(i!=band_number){
		par(new=T)
	}
}
...
currents_k <- arrary(0, dim=c(band_number,div_num_ka))
for(i in 1:band_number){
currents_k[i,] <- elementary_charge / Plank_constant * band_width[i] *lattice_constant_a_of_GaSe *(-sin(ka_vector)) /2 *flipping_numbers[i] 
}

#dipole transition matrix
Rabi_energies_max <- array(0,dim=c(band_number, band_number))

dipole_transition_matrix <- Rabi_energies_max / E_amplitude_max

1.2 *1.60218*10^-19 #1.2eV; converstion from eV to J
...
saveGIF({
  ...
  for (i in seq(1,(div_num_t+1), by=25)) {
...
plot(t_vector*10^15, Re(E_HHG), xlab="t (fs)", ylab="Electric field (arb. unit)", ylim=E_HHGrange, axes=FALSE, yaxt="n", col="red", type="l")  
par(new=T)
plot(t_vector*10^15, Re(E_vector), xlab="", ylab= "", ylim=Erange, col="black", type="l")  
#par(new=T)
...
frange <- c(0, 20)
Prange <- c(1, 10000000000000)
png("E_spectrum.png", width = 700, height = 500)  
plot(frequency/carrier_frequency, abs(E_spectrum)^2/100000000000, xlim=frange, ylim=Prange, log="y", xlab="f/carrier frequency", ylab="power (arb. unit)",col="black", type="l")  
par(new=T)
plot(frequency/carrier_frequency, abs(E_HHG_spectrum)^2/1000, xlim=frange, ylim=Prange, log="y", axes=FALSE, yaxt="n",xlab="", ylab= "", col="red", type="l")  
